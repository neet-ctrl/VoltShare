...
private fun BackupSettingsScreenContent(
    ...
    Scaffold(
        ...
        if (showSyncDetailsDialog) {
            val lastSync = if (uiState.lastSyncMillis == 0L) {
                strings.backupSyncDetailsNever
            } else {
                buildString {
                    append(
                        strings.backupSyncDetailsLastSuccessful
                            .format(TwLocale.formatDateTime(Instant.ofEpochMilli(uiState.lastSyncMillis)))
                    )
                    append("\n")
                    append(
                        strings.backupSyncDetailsRelative
                            .format(TwLocale.formatDuration(uiState.lastSyncMillis))
                    )
                }
            }
            val body = buildString {
                ...
                append("\n\n")
                append(
                    if (uiState.tokenCount >= 0) {
                        strings.backupSyncDetailsTokens.format(uiState.tokenCount)
                    } else {
                        strings.backupSyncDetailsCountsUnavailable
                    }
                )
                if (uiState.secretEntryCount >= 0) {
                    append("\n")
                    append(
                        strings.backupSyncDetailsSecretEntries
                            .format(uiState.secretEntryCount)
                    )
                    if (uiState.secretTrashEntryCount > 0) {
                        append("\n")
                        append(
                            strings.backupSyncDetailsSecretTrash
                                .format(uiState.secretTrashEntryCount)
                        )
                    }
                }
            }
            InfoDialog(
                ...
                title = strings.backupSyncDetailsTitle,
                body = body,
            )
        }
    }
