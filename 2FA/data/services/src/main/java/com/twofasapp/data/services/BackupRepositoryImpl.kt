    ...
    override suspend fun getCloudBackup(password: String?): CloudBackupGetResult {
        return when (val result = googleDrive.getBackupFile()) {
            is GoogleDriveFileResult.Success -> {
                ...
                try {
                    // New Drive files use the same universal container as the
                    // manual universal backup. Decode old token-only files as
                    // a fallback so existing accounts remain restorable.
                    val universalBackup = runCatching {
                        UniversalBackupCodec.decode(json, result.fileContent)
                    }.getOrNull()
                    val tokenBackup = universalBackup?.tokenBackup ?: result.fileContent
                    val backupContent = json.decodeFromString<BackupContent>(tokenBackup)
                    remoteBackupStatusPreference.put {
                        ...
                        if (password.isNullOrEmpty() && remoteBackupKeyPreference.get().isSet().not()) {
                            ...
                            try {
                                CloudBackupGetResult.Success(
                                    backupContent = decryptBackupContent(
                                        backupContent = backupContent,
                                        password = password,
                                        keyEncoded = remoteBackupKeyPreference.get().keyEncoded,
                                    ),
                                    secretsBackup = universalBackup?.secretsBackup,
                                    secretsUpdatedAt = universalBackup?.secretsUpdatedAt ?: 0L,
                                )
                            } catch (e: Exception) {
    ...
    override suspend fun updateCloudBackup(
        ...
        return withContext(dispatchers.io) {
            try {
                ...
                val serializedTokenBackup = serializeBackupContent(
                    backupContent.copy(updatedAt = updatedAt)
                )
                val secretKeyEncoded = backupContentCreateResult.keyEncoded
                val secretStats = secretsBackupProvider.backupStats()
                val syncedTokenCount = backupContent.services.size
                val serializedUniversalBackup = UniversalBackupCodec.encode(
                    json = json,
                    tokenBackup = serializedTokenBackup,
                    secretsBackup = secretsBackupProvider.exportBackup(
                        password = password,
                        keyEncoded = secretKeyEncoded,
                    ),
                    passwordProtected = backupContent.isEncrypted,
                    secretsUpdatedAt = secretsBackupProvider.backupUpdatedAt(),
                )

                when (val updateResult = googleDrive.updateBackupFile(
                    serializedUniversalBackup
                )) {
                        ...
                        if (backupContent.isEncrypted) {
                            ...
                            remoteBackupStatusPreference.put {
                                it.copy(
                                    ...
                                    syncedTokenCount = syncedTokenCount,
                                    syncedSecretEntryCount = secretStats.entryCount,
                                    syncedSecretTrashEntryCount = secretStats.trashEntryCount,
                                )
                            }
                        } else {
                            remoteBackupStatusPreference.put {
    ...
    override fun observeCloudBackupStatus(): Flow<CloudBackupStatus> {
        return remoteBackupStatusPreference.flow(emitOnSubscribe = true).map {
            CloudBackupStatus(
                ...
                syncedTokenCount = it.syncedTokenCount,
                syncedSecretEntryCount = it.syncedSecretEntryCount,
                syncedSecretTrashEntryCount = it.syncedSecretTrashEntryCount,
            )
        }
    }
