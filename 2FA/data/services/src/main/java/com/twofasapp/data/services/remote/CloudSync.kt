...
class CloudSync(
    ...
    suspend fun execute(trigger: CloudSyncTrigger, password: String?): CloudSyncResult {
        ...
        val syncBackupStatus = when (trigger) {
            ...
            CloudSyncTrigger.SecretsChanged,
    ...
    private suspend fun sync(
        ...
        when (remoteStatus) {
            is RemoteStatus.Success -> {
                if (shouldImportRemoteSecrets(remoteStatus, isFirstConnect)) {
                    val remoteKey = remoteBackupKeyPreference.get()
                    val imported = secretsBackupProvider.importBackup(
                        bytes = remoteStatus.secretsBackup!!,
                        password = password,
                        keyEncoded = remoteKey.keyEncoded.ifEmpty { null },
                    )
                    if (!imported) {
                        return RemoteStatus.Error(CloudSyncError.DecryptUnknownFailure)
                    }
                }
        ...
        return when (val result = backupRepository.getCloudBackup(password)) {
            is CloudBackupGetResult.Success -> {
                RemoteStatus.Success(
                    ...
                    secretsBackup = result.secretsBackup,
                    secretsUpdatedAt = result.secretsUpdatedAt,
                )
    ...
    private suspend fun shouldImportRemoteSecrets(
        remoteStatus: RemoteStatus.Success,
        isFirstConnect: Boolean,
    ): Boolean {
        val remoteSecrets = remoteStatus.secretsBackup ?: return false
        val localUpdatedAt = secretsBackupProvider.backupUpdatedAt()
        return remoteStatus.secretsUpdatedAt > localUpdatedAt ||
            (isFirstConnect && localUpdatedAt == 0L && remoteSecrets.isNotEmpty())
    }
}